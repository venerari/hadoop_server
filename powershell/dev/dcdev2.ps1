...
az vm run-command invoke -g $resourceGroup -n $vmName --command-id RunPowerShellScript --scripts '@create_dcdev1.ps1' --parameters 'arg1=Clark@12345!' --verbose

# dc1 reboot will happen in 6-10 min 
 
az vm run-command invoke -g $resourceGroup -n $vmName --command-id RunPowerShellScript --scripts '@create_dcdev2.ps1' --verbose
